  ...
  return (
    <div className="flex min-h-dvh flex-col">
      <header className="flex items-center justify-between px-5 pb-2 pt-5">
        <span className="text-xl font-bold tracking-tight text-fg">히스토리</span>
      </header>
      ...
      <main className="flex-1 px-5 pb-32">
        {stats.total === 0 ? (
          <div className="mt-24 text-center">
            ...
            <h2 className="text-lg font-bold text-fg">아직 지난 일정이 없어요</h2>
            <p className="mx-auto mt-2 max-w-[260px] text-sm leading-relaxed text-muted">
              약속을 마치고 Safe! 를 누르면 여기에 하나씩 쌓여요.
            </p>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            <section className="card-elev rounded-3xl border border-border bg-surface p-6 text-center">
              <p className="text-sm font-semibold text-muted">정시·일찍 도착률</p>
              <p className="mt-1 text-5xl font-bold text-brand">{stats.rate}%</p>
              <p className="mt-2 text-sm text-muted">
                총 {stats.total}번 중 {stats.onTime}번 제시간에 도착했어요
              </p>
            </section>

            {/* 요약 타일 */}
            <section className="grid grid-cols-3 gap-3">
              <StatTile value={`${stats.streak}`} label="연속 정시" />
              <StatTile value={`${stats.onTime}`} label="정시 도착" />
              <StatTile value={`${stats.late}`} label="지각" />
            </section>

            {/* 최근 일정 */}
            <section>
              <h2 className="mb-3 px-1 text-sm font-bold text-muted">최근 일정</h2>
